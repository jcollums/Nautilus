...
namespace Nautilus
{
    public partial class PackCreator : Form
    {
        ...
        private static bool cancellationEncountered = false;
        ...
        private void populatePacks()
        {
            ...
            foreach (var file in inputFiles)
            {
                ...
                {
                    // Cut the current pack off here before adding more files
                    inputFilePacks.Add(new List<string>(currentPackFiles));
                    currentPackFiles.Clear();
                    currentPackSize = 0;
                }

                currentPackFiles.Add(file);
                currentPackSize += fileSize;
                inputFilesTotalSize += fileSize;
            }

            // Add any remainder files
            if (currentPackFiles.Count > 0)
            {
                inputFilePacks.Add(currentPackFiles);
            }

            if (inputFilePacks.Count > 1)
            {
                var sizeInGB = (decimal)inputFilesTotalSize / GB;
        ...
        private bool extractRBFiles()
        {
            ...
            // The total number of files processed so far including previous packs
            var counter = currentPackIndex > 0 ? inputFilePacks.Take(currentPackIndex).Sum(pack => pack.Count) : 0;
            // The number of files processed in the current pack
            var packCounter = 0;
            // The total number of files processed successfully
            var success = 0;
        ...
        private bool hasAvailableSpace()
        {
            try {
                ...
                long tempDriveRequiredSpace = 0;

                string tempFilesWarningText = "";
                bool isSameDrive = targetDrive.Name == tempFilesDrive.Name;
                bool keepTempFiles = extractedFilesOption != DeleteExtracted;
                long singlePackOverhead = Math.Min(tempDriveRequiredSpace, MAX_PACK_SIZE);

                if (isSameDrive)
                {
                    if (keepTempFiles) {
                        targetDriveRequiredSpace *= 2;
                    } 
                    else
                    {
                        targetDriveRequiredSpace += singlePackOverhead;
                ...
                else
                {
                    if (keepTempFiles)
                    {
                        tempDriveRequiredSpace = targetDriveRequiredSpace;
                    } 
                    else
                    {
                        tempDriveRequiredSpace = singlePackOverhead;
                    }
                }
            ...
            {
                // Drive might be temporarily unavailable
                Log("Failed to evaluate amount of available disk space, proceed with caution.");
                Log(e.Message);
                return true;
            }
        }
        ...
        private void EnableDisable(bool enabled)
        {
            ...
            // Avoid disabling the cancel button
        ...
        private void btnBegin_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (fileOutput.ShowDialog() == DialogResult.OK)
                {
                    ...
                    {
                        Log("User cancelled the operation due to low disk space");
                        enableBeginUI();
                        return;
                    }
                    populatePackPaths();
        ...
        {
            if (inputFilePackPaths.Count == 1) {
                var xExplorer = new CONExplorer(Color.FromArgb(34, 169, 31), Color.White);
                xExplorer.LoadCON(inputFilePackPaths[0]);
                xExplorer.Show();
            }
            else {
                // If there's multiple packs we'll just open file explorer to the location of the packs, it might cause the user's computer to become unresponsive otherwise
                Process.Start("explorer.exe", $"/select,{inputFilePackPaths[0]}");
            }
            Dispose();
        }
        ...
        private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            if (inputFilePacks.Count > 1)
            {
                ...
                if (File.Exists(xOut))
                {
                    var response = MessageBox.Show($"The file {xOut} already exists, do you want to skip to the next pack?", Text + " - File Exists", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
        ...
        {
            if (cancellationEncountered || string.IsNullOrWhiteSpace(sOpenPackage))
            {
